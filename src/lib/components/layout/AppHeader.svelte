...
{#if variant === 'compact'}
	<section class="pt-2 sm:pt-5 lg:pt-6">
		<div
			class="glass-panel--soft relative overflow-hidden rounded-[1.6rem] px-3.5 py-3.5 sm:rounded-[1.9rem] sm:px-5 sm:py-4"
		>
			<div class="pointer-events-none absolute inset-0 -z-10">
				<div
					class="absolute inset-0 bg-[linear-gradient(135deg,rgb(var(--hero-card-start)/0.82)_0%,rgb(var(--accent-gold)/0.12)_52%,rgb(var(--hero-card-end)/0.74)_100%)]"
				></div>
				<div
					class="absolute right-[-12%] top-[-70%] h-44 w-44 rounded-full bg-[radial-gradient(circle_at_center,rgb(var(--hero-glow-primary)/0.28),rgba(255,255,255,0))] blur-3xl"
				></div>
			</div>
			...
			<div class="relative flex flex-col gap-3.5">
				<div class="flex items-start justify-between gap-3">
					<div class="min-w-0">
						<p
							class="text-[10px] font-semibold uppercase tracking-[0.22em] text-primary-500/80 sm:text-[11px]"
						>
							{$t('app.brand_global')}
						</p>
						<p class="mt-1 truncate text-lg font-semibold text-on-surface sm:text-xl">
							{$t('app.title')}
						</p>
					</div>
					...
					<div class="hidden items-center gap-2 sm:flex">
						...
						>
							...
							<label class="sr-only" for="compact-theme-select">{$t('app.theme_label')}</label>
							<select
								id="compact-theme-select"
								class="cursor-pointer appearance-none bg-transparent text-[10px] font-semibold uppercase tracking-[0.18em] text-on-surface outline-none focus-visible:text-primary-600 sm:text-[11px] sm:tracking-[0.2em]"
								value={$theme}
								on:change={(event) =>
									setThemePreference(
				...
				<div class="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
					<div class="flex flex-col gap-2 sm:flex-row sm:items-center">
						<button
							class="btn-secondary inline-flex w-full items-center justify-center gap-2 rounded-[1.2rem] px-4 py-2.5 text-sm font-semibold sm:w-auto sm:rounded-full"
						...
						{#if $canInstall && !$isStandalone}
							<button
								class="btn-secondary inline-flex w-full items-center justify-center gap-2 rounded-[1.2rem] px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.16em] text-primary-600 sm:w-auto sm:rounded-full sm:px-3.5"
								type="button"
								on:click={installApp}
							>
								{$t('app.install_cta')}
